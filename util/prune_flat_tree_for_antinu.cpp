...
TNtuple* ntload(const char* fname, const char* nout) {
  ...
  std::map<std::string, std::map<double,int>> reactor_norms;
  ...
  for (ie=0; ie<Nevents; ++ie) {
    ...
    /*std::vector<std::string>::iterator it =std::find(reactorNames.begin(), reactorNames.end(), reactorName);
    if (it == reactorNames.end()){
      std::cout<<"New Reactor! - "<<reactorName<<std::endl;
      reactorNames.push_back(reactorName);
      reactorNorms.push_back(0);
    }
    int index = std::distance(reactorNames.begin(), it);
    reactorNorms[index] += 1;
    */
    //std::map<std::string,int>::iterator it =std::find(reactorNames.begin(), reactorNames.end(), reactorName);
    //if (it == reactorNames.end()){
    //std::cout<<"New Reactor! - "<<reactorName<<std::endl;
    //reactor_norms[reactorName] += 1;
    //}
    ...
    reactor_norms[reactorName][fDistance] += 1;
  ...
  //for (int i = 0; i<abs(reactorNames.size()); i++)
  //std::cout<<reactorNames[i]<<"- Norm = "<<reactorNorms[i]<<std::endl;
  std::map<std::string, std::map<double,int>>::iterator it;
  std::map<double, int>::iterator sub_it;
  for(std::map<std::string, std::map<double,int>>::iterator it= reactor_norms.begin(); it != reactor_norms.end(); ++it){
      
    }
  //for(std::map<std::string, int>::iterator it = reactor_norms.begin(); it != reactor_norms.end(); ++it)
    std::cout<<it->first<<"- Norm = "<<it->second<<std::endl;
